...
class AVRoomMulti {
 ...
 public:
  ...
  struct Delegate {
    ...
    */
    virtual void OnRoomDisconnect(int32 reason) = 0;
  ...
  struct EnterParam {
    ...
    uint32 relation_id; ///< 业务侧的房间id，也就是由业务侧创建并维护的房间id，区别于SDK侧的房间id。该id可以是业务侧的讨论组号、群号、座位号等一切用于区分每个音视频通话的id。
  ...
  */
  ...
  typedef void(*ChangeCompleteCallback)(int32 ret_code, void* custom_data);
  ...
  @return 返回操作结果。
  */
  virtual int32 ChangeAuthority(uint64 auth_bits, const std::string& auth_buffer, ChangeCompleteCallback callback,
                                void* custom_data) = 0;
  ...
  /**
    ...
    @remark 修改角色不包括修改音频场景，音频场景仍然需要在进入房间前指定而且进入房间以后不能修改

    @return 返回操作结果。
    */
  virtual int32 ChangeAVControlRole(const std::string& av_control_role, ChangeCompleteCallback change_role_callback, void* custom_data) = 0;
